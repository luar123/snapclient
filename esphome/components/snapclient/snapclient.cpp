 ...
 *
 */
extern "C" esp_err_t audio_set_mute(bool mute) {
  if (!board_handle) {
...
static void http_get_task(void *pvParameters) {
  ...
  while (1) {
    ...
    scSet.buf_ms = 0;
    scSet.codec = NONE;
    scSet.bits = (i2s_data_bit_width_t)0;
    scSet.ch = 0;
    scSet.sr = 0;
    scSet.chkInFrames = 0;
    scSet.volume = 0;
                ...
                case SNAPCAST_MESSAGE_CODEC_HEADER: {
                  switch (internalState) {
                    ...
                    case 3: {
                      ...
                      p_tmp = (char*)malloc(typedMsgLen + 1);  // allocate memory for
                                                        // codec string
                      if (p_tmp == NULL) {
                        ESP_LOGE(TAG,
                                 "couldn't get memory "
                    ...
                    case 8: {
                      ...
                      p_tmp = (char*)malloc(typedMsgLen);  // allocate memory for
                                                    // codec string
                      if (p_tmp == NULL) {
                        ESP_LOGE(TAG,
                                 "couldn't get memory "
                ...
                case SNAPCAST_MESSAGE_SERVER_SETTINGS: {
                  switch (internalState) {
                    ...
                    case 4: {
                      ...
                      p_tmp = (char*)malloc(typedMsgLen + 1);
...
void SnapClientComponent::setup(void) {
  ...
  // ensure there is no noise from DAC
  {
    ...
    gpio_config(&gpioCfg);
    gpio_set_level((gpio_num_t)pin_config0.mck_io_num, 0);
    gpio_set_level((gpio_num_t)pin_config0.data_out_num, 0);
    gpio_set_level((gpio_num_t)pin_config0.bck_io_num, 0);
    gpio_set_level((gpio_num_t)pin_config0.ws_io_num, 0);
